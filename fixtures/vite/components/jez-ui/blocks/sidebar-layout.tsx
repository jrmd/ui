...
export function SidebarLayout({
  ...
  return (
    ...
    >
      <aside
        className={cn(
          "absolute inset-y-0 left-0 z-30 flex flex-col border-r border-border bg-background p-3 md:relative md:flex",
      ...
      >
        ...
        <nav aria-label="Workspace pages" className="mt-6 grid gap-1">
          {links
            .filter(
              (l) => rail || l.name.toLowerCase().includes(query.toLowerCase()),
            )
            .map(({ name, icon: Icon }) => (
              <button
                key={name}
              ...
              >
                ...
                {!rail && (
                  <>
                    ...
                    {name === "Inbox" && <span className="text-xs">4</span>}
                  </>
                )}
              </button>
            ))}
