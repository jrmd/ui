...
function useAlternatingFeatureStoryModel({
...
const AlternatingFeatureStoryCompositionContext =
  React.createContext<ReturnType<
    typeof useAlternatingFeatureStoryModel
  > | null>(null);
function useAlternatingFeatureStoryComposition() {
  const context = React.useContext(AlternatingFeatureStoryCompositionContext);
  if (!context)
    throw new Error(
      "AlternatingFeatureStory parts must be inside AlternatingFeatureStory.",
    );
  return context;
}
export function AlternatingFeatureStory(props: AlternatingFeatureStoryProps) {
  const model = useAlternatingFeatureStoryModel(props);
  const { className, rootProps, children } = model;
  return (
    <AlternatingFeatureStoryCompositionContext.Provider value={model}>
      <section
        {...rootProps}
    ...
    </AlternatingFeatureStoryCompositionContext.Provider>
  );
}
...
export function AlternatingFeatureStoryOverview({
  children,
  ...props
}: Partial<React.ComponentProps<typeof AlternatingFeatureStoryContent>> & {
  children?: React.ReactNode;
}) {
  const { heading, expanded, setExpanded } =
    useAlternatingFeatureStoryComposition();
  return (
    <AlternatingFeatureStoryContent {...props}>
      {children === undefined ? (
        <>
          <div>
...
export function AlternatingFeatureStoryActivity({
  children,
  ...props
}: Partial<React.ComponentProps<"div">> & { children?: React.ReactNode }) {
  const { features } = useAlternatingFeatureStoryComposition();
  return (
    <div
      {...props}
      className={cn(
